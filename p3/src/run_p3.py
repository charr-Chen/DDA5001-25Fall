...
# Run generation with different temperatures then evaluation
models = [
...
for model in models:
    for exp in experiments:
        # inference
        inference_output_dir = f"outputs/inference"
        ...
        inference_cmd = [
            ...
            f"--output_file={inference_output_dir}/{model['name']}-{exp['dataset']}-{exp['temperature']}.jsonl",
            f"--batch-size=16",
            f"--dp-size=1",
        ]
        ...
        # evaluation
        evaluation_output_dir = f"outputs/evaluation"
        os.makedirs(evaluation_output_dir, exist_ok=True)

        evaluation_cmd = [
            "python",
            "evaluate.py",
            f"--input_file={inference_output_dir}/{model['name']}-{exp['dataset']}-{exp['temperature']}.jsonl",
            f"--output_file={evaluation_output_dir}/{model['name']}-{exp['dataset']}-{exp['temperature']}-eval.jsonl",
        ]
