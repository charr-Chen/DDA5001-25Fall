...
def evaluate_results(input_file, output_file):
    ...
    with open(input_path, "r", encoding="utf-8") as f_in, open(
        ...
        for line_idx, line in enumerate(
            tqdm(f_in, total=total_lines, desc="Processing lines")
        ):
    ...
    print(
        f"Processing complete. Scored {lines_processed} lines across {num_problems} unique problems."
    )
    ...
    for problem_id, scores in problem2scores.items():
        ...
        for k in ks:
            ...
            if k > n_resps:
                ...
                try:
                    pass_at_k = 1.0 - math.comb(n_resps - c_correct, k) / math.comb(
                        n_resps, k
                    )
